...
class ForceServiceFactory {
    static create(propFileName) {
        ...
        def config = loadProperties '/default.properties'
...
class BulkMetadataManifestBuilder {
    ...
    BulkMetadataManifestBuilder(ForceService forceService, config) {
        ...
        this.config = config
		println " The value is :::${config['build.dir']}"
...
class XmlMergeTargetBuilder {
    ...
    private writeBuildXml() {
        ...
        builder.project('default': targetName) {
            'import'(file: '../ant-prop/setup-target.xml')
                    ...
                    parallel(threadCount: 4) {
                        filenames.each { filename ->
                            sequential {
                                ...
                                xmlmerge(dest: "$destDir/$filename", conf: 'ant-prop/xmlmerge.properties') {
                                    fileset(dir: srcDir) {
